...
`2026-09-01` · **P-04** · **The 29.90 NEES was a detector bug, not a covariance one.** `is_stationary` averaged the gyro magnitude over the 0.5 s window, so one sample of real rotation was divided by five: a car pulling away at 0.05 rad/s reads as `(4×0.0015 + 0.05)/5 = 0.011` rad/s and fires ZUPT **and** ZARU while turning. Exactly one such firing per pull-away, at the moment the stop has driven `P_bg` to its minimum, and it puts a systematic **8.0e-4 rad/s** — 2.6σ — into `b_g_z`. Invisible to every variance check (the spread about the ensemble mean was right to 20%); only a NEES, which does not subtract the mean, sees it. Max instead of mean: ZUPT+ZARU NEES 18.996, in band, asserted. · D-057

`2026-09-01` · **P-04** · D-053's second proposed cause — the same gyro sample used as process noise and as ZARU's measurement — **is not a cause**. Drawing an independent gyro read for ZARU moves the gyro-bias block from 8.98 to 9.15, the wrong direction. The `zaru_sigma` half of D-053 was real and worth 6 points (29.90 → 23.705 measured here, against the 23.71 D-053 predicted). Recorded because the handover-audit precedent applies to our own numbers too: a measured hypothesis that fails is worth more written down than quietly dropped. · D-056, D-057

`2026-09-01` · **P-04** · The NHC-inclusive NEES is **under**-confident (14.141 against [16.843, 19.195]) and matching `R_NHC` to the scenario's real residual makes it far worse, not better: at 5.49e-4 m/s the full state reads **6216**. The scenario's NHC residual is a deterministic model error, not white noise, so treating it as noise lets the filter accumulate unearned certainty. **P-10 owns the consequence:** a learned head driven toward the apparent residual fails exactly this way, and Gate 2's calibration criterion is what would catch it. · D-058

`2026-09-01` · **P-04** · **Scope note, three files.** (1) `is_stationary` is outside P-04's "FilterConfig and initialisation only" line but is where the NEES failure lives; the phase cannot close without it. (2) `tests/test_se23_derivation.py` was edited as `test_zaru_sigma_is_below_the_gyro_noise_the_repo_measured` instructs P-04 to — it is replaced by the three assertions it names, and `_nees_run`/`_mean_nees` now return per-block marginals from one cached sweep so the added assertions do not each cost a Monte-Carlo. (3) `test_propagate_linearises_at_the_midpoint_not_the_step_start` (P-02's) now pins its own flat `1e-3·I` prior: the separation it measures runs through `A`'s bias columns and so scales with the prior's bias blocks, and under the per-block `P₀` it falls to 1e-7 of a max|P| that a 9 m² position block now dominates. The threshold and the assertion are unchanged. All three flagged for seat S review. · reported here

`2026-09-01` · **P-04** · `eval/allan.py`'s `find_stationary_segments` **deliberately not** changed to match D-057, and its docstring is now stale where it says the criterion is "the filter's own": the finder keeps the mean statistic, so it is now the *looser* of the two. Changing it would move which segments Q was characterised on, and `eval/figures/allan_*` cannot be regenerated without the dataset. `tests/test_allan.py::test_segment_finder_agrees_with_the_filters_own_detector` still passes — its synthetic quiet window is 48× inside the threshold on the max — but the guarantee it asserts no longer holds by construction on real data. **Seat D's:** either mirror the max statistic and regenerate the artefacts on the dev box, or record why the finder should stay loose. · reported here

`2026-09-01` · **P-04** · **R-8 is NOT done and P-04 is therefore partial.** The accelerometer sign convention needs `S-T2[31422:36490]` or `S-T7[47809:52285]`, and `data/` in this container holds only `README.md` and `manifest/`. Until it is checked on the dev box, SE₂(3) test 2 ("stationary 60 s → drift < 1 mm") is a gravity-sign check under an *assumed* input convention, and `docs/SE23_PROPAGATION.md` §10 keeps it open. Half an hour with the bytes; it silently invalidates a Gate 1 result if skipped. · reported here
