...
`2026-09-02` · **P-01/split** · **The D-044 re-pick is made — D-092.** `LONG_OUTAGE` `S3a, Vtb3, Vta1a` → **`S3a, S3c, Vta1a, Vta1b, Vta16, Vw2, Vw4`**. Gate 1's 60 s criterion goes from **42 windows on one sequence to 459 on seven**, and off a stem that is one of only three with 1 Hz GNSS onto a set whose majority carries the 9 s cadence. Every stem passed two gates, not one: `align_to_sequence` (35 of 72) **and** `imu_stream`/`assert_uniform_grid` (31 of those 35). The second caught `Vta17`, whose sample interval strays 1.184 s and which the first had passed — pairing and integrability are separate questions and only checking the first would have promoted it. · D-092

`2026-09-02` · **P-01/split** · **`assert_no_family_straddles_the_split` fired on its first run, against a leak this re-pick had just introduced** — `Vw16a` in TRAIN, `Vw16b` held out, two segments of one drive. Lettered stems are segments of the same parent recording, so `assert_split_disjoint` (which compares stems) cannot see it, and a model that memorised the drive would score well on the held-out segment with the split still looking clean. Consequences taken rather than argued away: `S3c` left TRAIN to follow `S3a`, `Vta1b` joined the held-out set to follow `Vta1a`, `Vw14a/b/c` stayed whole in TRAIN, and `Vw16a` is unassigned — nine 10 s windows forgone. · D-092

`2026-09-02` · **P-01/split** · **`TRAIN` is pinned to nineteen named stems.** EVALUATION.md §3 had it as five "plus Vta/Vtb/Vw/Vfa/Vfb subsets not listed above" — an open-ended clause in prose only, so promoting `Vw4` out of that unnamed remainder would have overlapped train and test with **no check in this repo able to see it**. The clause is now an enumeration and `assert_split_disjoint` is meaningful for the first time. · D-092

`2026-09-02` · **P-01/split** · **Two things the new split cannot report, stated now rather than found later.** (1) **Wet road is empty** — `Vtb8` and `Vtb11` are refused at 33.7 m and 35.1 m, and no other stem is documented as wet; the group is kept as an empty tuple so the hole is visible in code. The `MAX_TRUTH_LAG_S` limit was **not** widened to keep it alive, which would have admitted the whole `Vtb` family at its systematic +1.6–2.1 s lag. (2) **The mandatory roundabout plot cannot be produced** — `Vta11` is 51.0 s and `REPLAY_LENGTH_S` is 60 s, so it left `MANDATORY_PLOT_SEQUENCES` in favour of `Vw4`. Closing it needs a scenario-length replay window or a longer recording; both are protocol decisions and neither was taken here. · D-092, EVALUATION.md §3

`2026-09-02` · **P-01/split** · Nothing was promoted into `CHALLENGING` and that is deliberate: every group name is a claim about what the vehicle was doing, the only source is the paper's list in EVALUATION.md §3, and **no document in this repo describes the 53 previously unallocated stems**. Filling a thinned group from them would be inventing the label (G-4). Promotion was confined to `LONG_OUTAGE`, where membership is a measurable property — pairs, integrates, long enough. · D-092

`2026-09-02` · **P-07** · **D-089's failure, from a second cause the enlarged split found on the first sweep.** `S3c` has one 10 s window (sample 36300) whose eleven truth epochs all carry the identical fix `(52.4074159, -1.4976300)` — a parked vehicle, verified as real and not a loader defect. `drift_percent` refused to divide by a zero path length, and because it raised a bare `ValueError` the exception escaped `evaluate_sequence` and **killed the run after S3a had already produced 1191 results**. Its own message already said such a window "must be excluded from the sweep, not divided by", and nothing implemented that — the same gap between an error's instruction and the code, in a second place. Fixed by extending D-089's mechanism rather than adding a second one: `ZeroDistanceOutage` is a **named** subclass so the catch cannot swallow `evaluate_outage`'s other `ValueError`s, which are real defects (shape mismatch, empty yaw, unhandled CRSE convention) and are pinned by a test that asserts a shape mismatch is *not* catchable as a dropped window. · D-093

`2026-09-02` · **P-07** · Worth checking before dropping anything from a Gate 1 statistic: **the zero-distance exclusion cannot flatter the result.** A stationary window is the easiest case in the sweep — ZUPT holds the filter almost exactly and the zero-order-hold GNSS baseline is exactly right — so it leaves the numerator and the denominator of a ratio of medians together. It is dropped because drift-% has no value at `L = 0`, and would have been dropped even if keeping it helped. · D-093

`2026-09-02` · **P-07** · **Gate 1 is still not measurable, and the blocker is now identified: `run_filter` never initialises the filter.** `eval/run.py` does `f = InEKF(cfg)` and starts propagating — attitude `R = I`, mount `R_sv = I`, `v = 0`, `p = 0`. **`pca_mount_yaw` is called in `tests/test_mount.py` and nowhere else**; D-073 built the initialiser and the returned spread for `P₀`'s mount block, and P-07 never wired it in. Measured consequence on real bytes, S3a, GNSS available throughout: `‖v‖` reaches **1223 m/s at 100 s** and 7486 m/s at 500 s — a sustained **~12.2 m/s², about 1 g** — and `‖p‖` reaches 1762 km. With `R_sv = I` the phone's cradle orientation is unmodelled, so gravity leaks into the horizontal axes and is integrated twice. · reported here

`2026-09-02` · **P-07** · The second half of the same failure, and the reason it presents as a crash rather than a bad number: once the state is that far off, **59 of 60 GNSS fixes are rejected by the χ² gate** (measured on S3a — offered 60, accepted 1), so the filter locks out its only correction and `P` runs away to **max 1.3e+70 with a minimum eigenvalue of −8.3e+53**, losing positive-definiteness. `np.linalg.inv` then raises `LinAlgError: Singular matrix` inside `update_zupt` on Vw4. This is the same lock-out mechanism D-057 measured for ZUPT — "legitimate innovations fall outside the gate and are locked out permanently" — reached from a different direction. · reported here

`2026-09-02` · **P-07** · **Every held-out stem diverges, not just the one that crashed.** max|P| after 600 s: S3a 1.03e+12, Vta1a 1.22e+12, Vw4 2.23e+11, Vw2 1.93e+11, Vw6 2.49e+07. Identical **with and without** GNSS updates applied, to the digit, which is what localises the fault to `propagate()` plus initialisation rather than to the update family. **Consequence for the sweep that did complete stems:** S3a's 1191 and Vw2's 2568 window results were produced by a diverged filter and are not a Gate 1 number — no ratio is computed or reported from them. The synthetic NEES tests do not catch this because `synthetic_drive` builds a level phone with `R_sv = I` genuinely correct, which is the one case where the missing initialisation is harmless. · reported here

`2026-09-02` · **P-07** · Two defects found and fixed on the way to that diagnosis, both of them the *same* gap between an error message's instruction and the code, in two more places: D-089 (no truth coverage) and D-093 (zero distance travelled). D-089 is now confirmed working on real bytes — `Vw12`, the stem that used to abort the entire sweep, dropped 2 windows and still produced 18 results, and `Vw2` dropped 1 of 2569. · D-089, D-093

`2026-09-02` · **P-07** · **The initialisation gap is closed and Gate 1 is still not measurable.** `initialise_filter` wires D-073's `pca_mount_yaw` into `run_filter` — it had been called from `tests/test_mount.py` and nowhere else. max|P| after 600 s on S3a: **1.03e+12 → 5.0e+04**. `state.R` is **phone**→NED, not vehicle→NED (`_h_vehicle_velocity` computes `v_veh = R_sv @ R.T @ v`, and `propagate` applies `R` to the raw phone accelerometer), so the attitude is `R_vn @ R_sv`; setting it to `R_vn` alone leaves the mount out of the gravity cancellation and was this change's own first attempt. All four candidate compositions were measured before one was chosen. · D-094

`2026-09-02` · **P-07** · **The live blocker, with the sequence measured.** The χ² gate still rejects 59 of 60 fixes on S3a: innovation **0.78 m** at fix 0 (accepted), **120.40 m** nine seconds later (accepted, χ² 9.3), **825 m** at fix 2 (rejected, χ² 10312), then state and innovation run away together while `σ_p` grows too slowly to re-admit anything. 120 m in 9 s is ~3 m/s² of unmodelled acceleration — an order of magnitude above the 0.105 m/s² levelling residual and 900× the 3.3 mm/s² accel-bias prior — so it is neither bias nor levelling. Seat S, inside Gate 1's fixed diagnosis order. Next suspects in order: the gyro axis mapping under real turning (D-047's `gyro_yaw` = device x), the initial velocity being a 30 s mean assigned to the window's opening instant, and `nhc_is_valid`'s gating on real dynamics. · D-094

`2026-09-02` · **P-07** · `synthetic_drive` gained the `GRAVITY` channel every real `S-` file ships. Without it the mount initialiser has no vertical and is skipped, so the fixture could not exercise the PCA path **at all** — silently. This is the same shape as D-087's finding about `t_rel_start_s`: a fixture that omits a column the real files carry cannot see a defect in the code that reads it. · D-094

`2026-09-02` · **P-07** · **The 120 m is the gyro axis mapping, and the correction is measured only in its vertical component.** `imu_stream` hands `[gyro_yaw, gyro_pitch, gyro_roll]` to `propagate` as device x/y/z; the vehicle's yaw rate is on `gyro_pitch`, so the filter integrates every turn as a **tilt**. On S3a the attitude wanders **87.0° in 60 s** and `R @ accel + GRAVITY_NED` goes from ‖mean‖ 0.075 / rms 1.11 m/s² with `R` frozen to ‖mean‖ **12.77** / rms **12.90 m/s²** with `R` gyro-propagated — ~1.3 g of gravity in the horizontal axes, integrated twice, and `‖v‖` reaches 42.72 m/s in 9 s. `gyro_pitch` wins by R² on **18 of 18** loadable stems. Not fixed: it contradicts D-085 on stems this container cannot fetch, and the horizontal pair is invisible to every test tried. · D-095, `eval/axis_check.py`

`2026-09-02` · **P-07** · **D-094's innovation table does not reproduce.** At `5cae714` on S3a this session measures fix 1 at **59.99 m, χ² 253.8, rejected**; D-094 records **120.40 m, χ² 9.3, accepted**, and fix 2 at 825 m against 932.94 m here. The mechanism and the conclusion are unchanged — the gate locks out and the state runs away — but the numbers are not the same ones, and D-094's χ² 9.3 implies a `σ_p` near 68 m where this run carries 2.18 m. Re-measured with `python -m eval.axis_check` and `scratchpad/` reproduction; the difference is unexplained. · D-095

`2026-09-02` · **P-07** · **`initialise_filter` assigns a 27 s mean velocity to t = 0, and it is a real but secondary error.** S3a's warmup fixes report 3.09 → 6.07 → 11.31 → 11.93 m/s; the first-to-last chord gives **7.85 m/s at a heading 57° from the first-two chord**, against a true opening speed of **3.09 m/s** — 4.76 m/s of velocity error, worth ~43 m over 9 s on its own. Measured *through the filter* it moves almost nothing (fix-1 innovation 60.0 → 66.1 alone, 19.2 → 17.5 alongside the axis correction), because ZUPT and NHC dominate the first window. **The stream already ships the instantaneous quantities**: `gps_speed_kmh` (m/s, D-096) and `gps_orientation_deg`, per fix, both on the allowlist — the chord is reconstructing badly what the receiver reports directly. Worth doing after the axis question is settled, not before. · reported here

`2026-09-02` · **P-07** · **The χ² gate is the remaining blocker and it is a covariance-realism question, not a threshold one.** With the axis correction applied, S3a's fix-1 innovation is 15–20 m while the filter's own `σ_p` is **2.19 m**; `Q_c` from D-045's Allan run predicts ~0.12 m of position spread over a 9 s propagation, so the filter is overconfident by **two orders of magnitude** against its actual error and rejects 9 of 10 fixes. Q models sensor noise and nothing else — not attitude error, not mount error, not the 15.63° PCA mount spread S3a's own initialiser reports. This is item three of Gate 1's diagnosis order and it must not be reached by widening `gnss_gate_chi2` or inflating Q, which would both make the symptom go away. Seat S. · reported here

`2026-09-02` · **handover-audit** · The seven commits the session handover described were **not** in the tree at session start — `HEAD` was `dedd964` and `5cae714` did not resolve. They exist: `git fetch origin` brought them back on `origin/claude/split-repick-truth-gaps-znt0a0`, and D-089/D-091/D-092/D-093/D-094 verify as described. The designated branch was based on that lineage rather than on `dedd964`. Recorded because the 2026-08-31 handover-audit finding makes "the claim is not in the tree" worth distinguishing from "the ref was not fetched". · reported here

`2026-09-02` · **P-05** · **Already done, and not redone.** `44f6b17` landed both baselines: `eval/baselines.py` carries `naive_strapdown` (the H-2 exception, named as such throughout) and `gnss_available`, both emitting `OutageMetrics` through `eval/metrics/core.evaluate_outage` with no parallel metrics path, both wired into `eval/run.py`'s `METHODS` tuple, and `tests/test_baselines.py` is 440 lines against hand-computed cases. The §7 checkbox is unticked because a human ticks it, not because work is outstanding. Verified rather than assumed: main's suite is 458 passed. P-06 consumes `naive_strapdown` as its comparison, which is the first use of P-05 outside its own tests. · reported here

`2026-09-02` · **P-06** · **The reproduction is trained; both blockers D-072 named are gone.** The IO-VNBD CSVs *are* reachable from a container like this one — `eval/fetch.py`'s `media.githubusercontent.com` route (D-097) pulled all 288 synchronised files, 818 MB, every one verified against the manifest's sha256 — and `torch` installs in about four minutes. D-059's "cannot fetch" is true of the LFS batch API and not of the media endpoint, which is what D-097 already recorded; noting it here because D-072 still reads as if the data were unobtainable. · D-100

`2026-09-02` · **P-06** · **The instructive half of the reproduction is the target that failed.** Asked to regress per-second travel directly from a 1 s IMU window, the network reached a validation MAE of 3.54 m — against 3.89 m for predicting the training mean with the input ignored entirely — and 53.7% median drift at 10 s. It was not undertrained: absolute speed is not in the input. Per-epoch travel has σ = 5–8 m over TRAIN; the epoch-to-epoch change in it has σ = 0.67–0.77 m, and the change is what an accelerometer integrates to. The same architecture, same data, same hyperparameters, regressing the change and integrating from the entry speed: MAE 0.275, drift 11.6%. **A network that has been asked for something not in its input does not fail loudly — it predicts the mean and converges.** · D-099

`2026-09-02` · **P-06** · `eval/run.py`'s `--data` defaults to `data/IO-VNBD` while calling `paired_truth_path(name, args.data)`, and `manifest_path_for` joins the manifest's own `IO-VNBD/`-prefixed paths onto that root — so the default cannot resolve and every truth lookup raises `TruthPairingError` unless the flag is passed as `--data data`. The trainer added here defaults to `data` and says why in a comment. Not fixed: `eval/` is out of P-06's scope and P-07 is live in that file. **Seat D.** · reported here

`2026-09-02` · **P-06** · The improvement-over-raw-INS ratio grows with outage length — 80.0% at 10 s to 97.7% at 180 s — because raw INS diverges quadratically and reaches 40.9 km of CRSE at 180 s, not because the network gets better; its own drift over the same span goes 11.6% → 47.8%. Worth stating wherever the ratio is quoted: it is the published claim's shape (89.55%) and it is the flattering half of the result. · D-100

`2026-09-02` · **P-04 / PR #6** · The P-04 branch this session opened is **closed as superseded** by `245ab07`, which reached the same root cause independently — one false ZARU per stop at the pull-away, where `is_stationary`'s window still holds four stopped samples and one moving one — and fixed it by χ²-gating ZARU rather than by changing the detector. Both sessions also independently measured that D-053's propagate/ZARU correlation hypothesis is *not* a cause. One substantive difference is left on the table and is recorded rather than ported: under main's fix a false **ZUPT** still fires at each pull-away, deliberately, because gating ZUPT was measured to make it worse (18.93 → 44.74 NEES) and because a false ZUPT at near-zero speed is bounded where a false ZARU's yaw rate is not. The alternative — `is_stationary` on the window maximum — makes the false stop undetectable rather than caught, but trades it for a stricter detector whose 0.02 rad/s threshold has not been re-checked against real idling data. · reported here

`2026-09-02` · **P-06** · **Scope note, two files outside `models/` and `DECISION_LOG`.** (1) `tests/test_baseline_training.py` is new — DoD #3 requires a test for new behaviour, and the audit half already set the precedent with `tests/test_baseline_audit.py`. (2) `tests/test_baseline_audit.py::test_running_without_audit_refuses_rather_than_training_on_unresolved_choices` asserted that `main` raises, which is exactly what implementing the phase removes; it is replaced by a test of the property underneath — that the entry point never returns a number it did not earn — with the supersession argued in D-100 and in the test's own docstring, per G-3. The sibling invariant it holds (a stated hyperparameter is never relabelled as ours) caught a real defect in this session's own row wording and was left alone. Flagged for seat M review. · D-100
