...
## Corpus Check
- 185 files · ~533,859 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3054 nodes · 5724 edges · 167 communities (138 shown, 29 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 373 edges (avg confidence: 0.68)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `19c96755`
...
- wrap_to_pi
- _nhc_mount_run
- test_no_operator_ui_surface_labels_a_drift_or_a_grade
...
Cohesion: 0.06
Nodes (73): BaselineTrajectory, course_over_ground(), epoch_times(), gnss_available(), initial_state_from_truth(), naive_strapdown(), ndarray, The two Gate 1 baselines: naive strapdown INS, and GNSS-available.  Gate 1 is (+65 more)
...
Cohesion: 0.04
Nodes (78): chi2_gate(), detect_mount_disturbance(), is_stationary(), nhc_is_valid(), Detect a stopped vehicle for ZUPT/ZARU.      Both conditions must hold: low ac, Whether the non-holonomic constraint may be applied this step.      NHC says t, Mahalanobis gate. True means *accept* the measurement.      This single functi, Detect the phone being knocked, so R_sv covariance can be re-inflated.      A (+70 more)
...
Cohesion: 0.13
Nodes (18): main(), assert_split_disjoint(), assert_split_is_loadable(), The frozen train/test split.  **Defined in code, not in a notebook.** A split, Every held-out sequence, deduplicated and ordered., No sequence may appear in both train and test.      Checked in CI. This is the, No split may name a sequence that has no smartphone stream.      A split entry, test_sequences() (+10 more)
...
### Community 8 - "InEKF"
Cohesion: 0.05
Nodes (39): InEKF, Error-state Invariant EKF on SE_2(3).      Sprint 1 (seat S) implements propag, Section 7.1. `H_vb = [0, R_hat^T, 0, 0, 0, 0]` -- the attitude column is exactly, Zero-velocity update. Observes accelerometer bias.          Uses section **7.1, Learned forward-speed pseudo-measurement with its predicted variance., The measured failure D-053 recorded and P-04 diagnosed: `is_stationary` averages, The asymmetry is measured, not assumed (D-057): gating ZUPT takes its NEES from, Placeholders raise. A stub that returns None would let the harness produce plaus (+31 more)

### Community 10 - "run.py"
Cohesion: 0.05
Nodes (84): FilterConfig, MountInit, NavState, Per-axis ZARU measurement sigma, read from the stop detector's own window (D-131, Tuning. Process noise is seeded from a measured Allan-variance run, never guesse, What the PCA initialiser found: the mount rotation, and how well it is pinned do, Nominal state. Covariance is carried separately as an 18x18 on the error state., Heading in radians. The quantity the whole error budget turns on. (+76 more)

### Community 11 - "_nees_run"
Cohesion: 0.11
Nodes (31): exp_so3(), slice, Rodrigues. Gamma_0 *is* the SO(3) exponential., Error-state Kalman update, with the correction **subtracted** (D-050)., Pack (R, v, p) into the 5x5 SE_2(3) matrix of section 2., se23_exp(), unpack(), x_of() (+23 more)
...
Cohesion: 0.09
Nodes (30): _drive_with_a_knock(), _driving_specific_force(), `R_sv` -- the PCA initialiser, in-filter estimation, and the bump detector (P-11, `spread_rad` is the standard error of the principal-axis angle, so it must fall, Accelerometer samples at 10 Hz are not independent draws, and using `n` would un, The loop D-055 left open: with an initialiser run, the mount prior is a measurem, A reset would *shrink* the covariance of a filter that had already lost track, w, Cruise at 15 m/s with a gentle yaw rate; knock the phone at t = 30 s; run to t = (+22 more)

### Community 15 - "inekf.py"
Cohesion: 0.08
Nodes (40): adjoint(), _cue_statistic(), _effective_samples(), gammas(), log_so3(), mount_yaw_from_dynamics(), _moving_average(), orthonormalise() (+32 more)

### Community 16 - "cadence.py"
Cohesion: 0.08
Nodes (45): build_parser(), CadenceRow, _fixed(), main(), measure_cadence(), ArgumentParser, Path, GNSS cadence, and whether the paired `V-` track can stand in as ground truth. (+37 more)
...
Cohesion: 0.12
Nodes (24): align_to_sequence(), load_truth(), Read lat, lon and time of day from one `V-` file.      The header is read on i, Compare a loaded `S-` sequence's own GPS fixes against the paired `V-` track., The failure a new folder layout produces must say what to add, not just that it, The check has to be sharp enough to catch the failure it exists for. At 16 m/s o, A pair that agrees only after a time shift is a finding about the dataset. Apply, `np.arange` accumulates float error, and a lag of -1.8e-14 in a committed artefa (+16 more)
...
Cohesion: 0.07
Nodes (36): assert_non_overlapping(), generate_outages(), generate_sweep(), mask_gnss(), Outage, ndarray, Synthetic GNSS-outage injection.  Protocol: docs/EVALUATION.md section 3. Leng, Verify the non-overlap guarantee. Cheap to check, expensive to discover violated (+28 more)

### Community 23 - "test_metrics.py"
Cohesion: 0.14
Nodes (17): cte(), Cumulative True Error: the *signed* sum of per-epoch errors, in metres.      S, Metric unit tests, each against a hand-computed case.  Every number in the sub, 60 s outage, 10 m/s, perfect heading, 2% under-prediction of distance.      Tr, Three epochs, each under-predicted by 1 m along-track.      Hand-computed: per, The pair exists precisely because they disagree: +2 and -2 cancel in CTE but not, There is no else-fallthrough in crse(): an unknown member must raise, not quietl, Eq. (16)'s root is inside the sum, which is the whole difference from CTE (Eq. 1 (+9 more)
...
### Community 29 - "main"
Cohesion: 0.05
Nodes (60): _last_sunday(), _parse_local_seconds(), Make a seconds-since-midnight series monotonic across a midnight rollover., Day of the month of its last Sunday. March and October both have 31 days., Seconds to subtract from a UK *local* timestamp to get UTC: `BST_OFFSET_S` or ze, Parse the `S-` `date` column into seconds since midnight, **as shipped**., Local seconds since midnight per row, **not** unwrapped. NaN where the row does, The same column, converted from UK local time to UTC -- the `V-` VBOX clock (D-0 (+52 more)
...
Cohesion: 0.05
Nodes (73): assert_uniform_grid(), evaluate_sequence(), fix_arrays(), imu_stream(), This stem cannot be graded, and the run says so rather than reporting a number a, Every window of every length for one sequence, all three methods.      One GNS, `(gyro, accel, t_rel_s)` for one sequence: `(n, 3)`, `(n, 3)`, `(n,)`.      Ax, Per-sample `dt`, refusing a stream that is not the 10 Hz grid the sweep assumes. (+65 more)
...
Cohesion: 0.11
Nodes (24): _canonicalise(), _distinct_fixes(), load_sequence(), load_split(), _preferred_copy(), DataFrame, Path, IO-VNBD "S-" smartphone-stream loader.  Every read passes through the leakage (+16 more)
...
Cohesion: 0.11
Nodes (19): D-041 claims 100% offline for the evaluation logger (android/). The harness and, D-080. A renderer has no business with a random number, and neither has a view t, The specific thing that was here: a `PREVIEW_TELEMETRY` constant, rendered whene, D-126: `TunnelFsm` decides when GNSS is suppressed and when a fix counts as veri, D-126 deviation 3 / D-115: the fix applied after N consecutive rejections is on, D-080 / R-B: TunnelCorridor must advance strictly on the estimator's pose clock, R-F / D-126: hazard chips are strictly limited to the declared set derived from, D-081 caption discipline: the speedometer HUD explicitly names its source ('filt (+11 more)
...
Cohesion: 0.15
Nodes (18): build_parser(), gate1_by_mount_class(), gate1_ratio(), ArgumentParser, Path, `{method: {length_s: summary}}`, each summary from `eval.metrics.core.summarise`, Gate 1 drift ratio and window counts partitioned by mount class (D-115)., The Gate 1 number: physics-only median drift over GNSS-available median drift, a (+10 more)
...
Cohesion: 0.11
Nodes (26): AxisMeasurement, candidate_gyro(), _course_and_gyro(), main(), measure(), ndarray, Measure which `S-` gyroscope column carries the vehicle's yaw rate, and what it, `(course_rate, gyro_mean, course_change, gyro_integral, keep)` over fix interval (+18 more)
...
Cohesion: 0.06
Nodes (31): 1. Clone the Repository, 1. Protocol Dry Run (No Dataset Required), 2. Create and Activate a Virtual Environment, 2. Full Benchmark Evaluation, 3. GNSS Cadence and Ground-Truth Verification, 3. Install Dependencies, Android Unit Tests, Architecture (+23 more)
...
Cohesion: 0.17
Nodes (12): initial_covariance(), `P0`, block by block, with every entry traceable to a section of docs/ERROR_BUDG, Re-express a covariance built on *plain* errors in the filter's right-invariant, right_invariant_from_plain(), The two must not drift apart: a `P0` nobody uses is documentation, not a prior., The harness passes the `S-` stream's own per-sample `gps_accuracy_m` where it ha, A diagonal `P0` in right-invariant coordinates is not a diagonal plain prior. Wi, test_p0_is_diagonal_because_a_prior_has_learned_no_correlations_yet() (+4 more)
...
Cohesion: 0.09
Nodes (35): find_stationary_segments(), Find stretches long and clean enough to compute an Allan curve on.      Statio, ndarray, Tests for the Allan-variance seeding of Q.  The estimator is checked against p, The old default, 3e-3 rad/s/sqrt(Hz), is 10.3 deg/sqrt(hr) -- outside the 0.5-5, S-I really does contain a 285 s hole. Allan variance assumes a uniform grid, so, The far side of S-I's pause logs at ~1 ms with 39% of samples sharing a timestam, Idling with an occupant passes the ZUPT detector and must still be excluded from (+27 more)

### Community 98 - "ValueError"
Cohesion: 0.14
Nodes (11): Widen the mount block of `P` after a detected knock, so `R_sv` is re-estimated., calibration_summary(), per_stem_summary(), Place a reset-clock prefix on the truth clock using only its own fixes., Path speed from truth positions at the actual samples in one input window., Coverage-first calibration and sharpness statistics, independent of torch., Apply :func:`calibration_summary` per held-out stem., _truth_clock_offset_for_prefix() (+3 more)
...
Cohesion: 0.17
Nodes (12): AssertionError, assert_no_family_straddles_the_split(), The parent recording a stem belongs to: `Vw14a` -> `Vw14`, `S3a` -> `S3`, `Vw4`, No parent recording may have one segment in TRAIN and another in the held-out se, stem_family(), Hold back complete TRAIN families using P-06's deterministic D-098 rule., validation_families(), Lettered stems are segments of one drive. `Vw14b` held out while `Vw14a` and `Vw (+4 more)
...
Cohesion: 0.10
Nodes (22): _approx_residuals_m(), _best_lag_s(), _nearest(), ndarray, A 10 Hz reference trajectory: time of day, latitude, longitude. Nothing else., Nearest-sample indices for a set of epoch times. Raises if any is further than t, Latitude/longitude at each epoch time, shape ``(n, 2)``., Local NED positions at each epoch time, relative to the first, shape ``(n, 2)``. (+14 more)
...
Cohesion: 0.24
Nodes (9): _evaluate_arrays(), ndarray, Per-channel 0--1 scaler fitted only from the training windows (D-071)., Concatenate a group of stems after selecting its target and valid delta labels., Run a fitted torch model and bring its two outputs back to NumPy., Fit a target-specific head and retain the weights with the lowest validation NLL, Scaler, _stack() (+1 more)
...
Cohesion: 0.07
Nodes (45): assert_feature_safe(), assert_no_leakage(), LeakageError, Channel allowlist and the leakage guard.  PS 26168 disallows wheel odometry. I, Raise LeakageError if any disallowed channel is present.      Called by the lo, Stricter guard for tensors entering a model or filter: inertial channels only., A disallowed channel reached a feature path.      Deliberately an AssertionErr, assert_truth_only() (+37 more)

### Community 133 - "parametrize"
Cohesion: 0.18
Nodes (6): OnyekpeBaseline, Tensor, Vanilla RNN correcting INS displacement error and orientation-rate error., Should land near the published ~8,200. A large divergence means the architecture, 0-1 feature scaling, fitted on the training windows **only** (D-071).      The, Scaler

### Community 134 - "Records"
Cohesion: 0.24
Nodes (9): git_sha(), make_stamp(), Provenance stamping.  Every figure and table in the submission carries the com, Current commit SHA, suffixed '-dirty' if the tree has uncommitted changes., Seed every RNG we use and return the stamp describing this run.      Call once, seed_everything(), test_different_seeds_diverge(), test_same_seed_gives_identical_draws() (+1 more)
...
### Community 148 - "train"
Cohesion: 0.14
Nodes (20): One-line stamp for a figure caption or table footer., Write the provenance line into a matplotlib figure itself.      In the figure,, stamp_figure(), audit_table(), build_parser(), _choose_target(), _export_fp16(), load_windows() (+12 more)
...
Cohesion: 0.20
Nodes (8): assert_features_are_clean(), build_parser(), Hyperparameter, _median(), ArgumentParser, Training entry point for the Onyekpe INS baseline reproduction (P-06).      py, Run the repo's own leakage guard over the exact columns the model is fed. Return, Median, not mean: the protocol reports drift % as median and p95 (EVALUATION.md)

### Community 156 - "test_a_constant_offset_is_annihilated"
Cohesion: 0.12
...
### Community 158 - "Section"
Cohesion: 0.50
Nodes (4): Section, NEARBY, RECENT, RESULTS

### Community 159 - "sample_rate_hz"
Cohesion: 0.50
...
### Community 160 - "summarise"
Cohesion: 0.50
Nodes (4): Aggregate across sequences.      Reports the **median and 95th percentile** of, summarise(), The mean hides the tail and the tail is what a judge finds., test_summary_reports_tail_not_just_mean()

### Community 161 - "wrap_to_pi"
Cohesion: 0.50
...
## Knowledge Gaps
- **540 isolated node(s):** `UNKNOWN`, `PEDESTRIAN`, `VEHICLE`, `GNSS`, `INS` (+535 more)
...
- **Why does `NavigationScreen()` connect `NavigationScreen.kt` to `SpeedHud.kt`, `NavigationRoute`, `glassmorphic`, `GeocodingService`, `TelemetryState`, `GuidanceBanner.kt`, `SearchItem`, `LocalNavigationEstimator`?**
  _High betweenness centrality (0.024) - this node is a cross-community bridge._
- **Why does `FilterConfig` connect `run.py` to `test_allan.py`, `test_se23_derivation.py`, `test_filter.py`, `test_baselines.py`, `InEKF`, `test_ffi_contract.py`, `_canonicalise`, `_nees_run`, `test_mount.py`, `inekf.py`, `GnssHub`, `detector_settings`, `allan.py`, `regress_heading_on_integrated_gyro`, `test_harness_wiring.py`, `normalise`?**
