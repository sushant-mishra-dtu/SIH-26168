...
| CTE, CRSE, drift %, yaw error | **working**, each against a hand-computed case | D |
...
| InEKF propagate / update | `NotImplementedError` — Sprint 1 | S |
...
1. **`S-` smartphone channels only.** Wheel speed (`V-`) is disallowed by PS 26168. A CI test fails
   on any `V-` column reaching a feature tensor — and is itself verified to reject.
