...
- **Stationary segments exist, but the longest is 8.4 min, not >20 min.** ⚠️ This bullet previously
  ...
  uniformly-sampled, still stretch is **484 s in `S-T2`**, followed by 426 s in `S-T7`, two in
  `S-A6` and one in `S-I` (507 / 448 s before D-120 excluded the second of settle and pull-away
  at each end; the S-I run clears the sampling checks only once its ends are trimmed). Nothing
  else clears 120 s, and none of the last three is quiet. They remain free ZUPT/ZARU ground truth and they are what the
