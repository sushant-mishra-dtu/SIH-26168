...
**Allan variance procedure:** log the phone stationary for several hours, temperature-stable, at max
rate (needs `HIGH_SAMPLING_RATE_SENSORS`). Compute the overlapping Allan deviation σ(τ) per axis.
The **−½ slope** region gives ARW/VRW (read σ at τ = 1 s); the **flat minimum** gives bias
instability (×0.664); the **+½ slope** gives rate random walk. Seed the InEKF process noise Q from
these instead of guessing. Repeat at a few temperatures to bound thermal drift.
