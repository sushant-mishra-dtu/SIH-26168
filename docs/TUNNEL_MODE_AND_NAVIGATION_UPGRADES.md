...
> **Status (13 Sep 2026).** §1.2's FSM is implemented — `android-ui/app/src/main/java/com/sih/idr/demo/backend/tunnel/TunnelFsm.kt`, decision row **D-126**, which lists five deliberate deviations from the diagram below (a reverse edge out of `PRE_ARMED_ENTRY`, a rejected fix resetting the pass counter rather than bouncing the state, forced acceptances recorded as such, a jitter-proof timeout, edge-triggered map inputs) and the two inputs it does not use (HDOP; no sub-second C/N₀ claim). Phase 1 of §5 is therefore done except for the on-device threshold measurement.
>
